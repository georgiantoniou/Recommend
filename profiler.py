...
class PerfEventProfiling(EventProfiling):
    def __init__(self, sampling_period=1, sampling_length=1, iteration=1):
        ...
        self.pid = os.popen("ps aux | grep cf_server | grep -v docker | awk '{print $2}' | head -1").read().strip().split(" ")[0]
        #self.pid = os.popen("ps aux | grep MicroSuite | grep -v docker | awk '{print $2}' | tail -2 | head -1").read().strip().split(" ")[0]
        #get pid of midtier server
...
def server(port,perf_iteration):
    perf_event_profiling = PerfEventProfiling(sampling_period=140,sampling_length=180,iteration=perf_iteration)
